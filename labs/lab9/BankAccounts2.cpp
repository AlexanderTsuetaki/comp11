                             ...
                             float interestRate)
{
	...
	(*temp).owner = owner;
	(*temp).accountNumber = accountNum;
	(*temp).balanceDollars = balanceDollars;
	(*temp).balanceCents = balanceCents;
	(*temp).interestFraction = interestRate/100;
        return temp;
}
/*
...
void BankAccount_print(BankAccount *accountp)
{
        ...
        cout << "BankAccount:  " << endl
             ...
             << "    interest = " << accountp->interestFraction *100 << "\n"
             << "  }"
             << endl;
}
...
float BankAccount_balance(BankAccount *accountp)
{
	return (*accountp).balanceDollars + 0.01*(*accountp).balanceCents;
}
...
void BankAccount_applyInterest(BankAccount *accountp)
{
	float temp;
	int dollars;
        temp = (*accountp).balanceDollars + 0.01*((*accountp).balanceCents);
	temp = temp + temp * ((*accountp).interestFraction);
	dollars = (int)temp;
	(*accountp).balanceDollars = dollars;
	(*accountp).balanceCents = (int)((temp-dollars)*100);
}
