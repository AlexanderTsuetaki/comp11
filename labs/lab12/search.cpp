...
int search_parallel_arrays(int nums[], string *words[])
{
		...
		if ( (words[i]) != nullptr and *(words[i]) == SECRET)
			sum += nums[i];
	}
        return sum;
}
...
int search_array_of_structs(Item list[])
{
	int sum = 0;
        for(int i = 0; i < ARRAY_SIZE; i++){
		if(list[i].wordptr != nullptr and *(list[i].wordptr) == SECRET)
			sum += list[i].num;
	}
...
int search_array_of_ptrs_to_structs(Item *list[])
{
        ...
        // YOUR CODE HERE
        for(int i = 0; i < ARRAY_SIZE; i++){
		if(list[i] != nullptr and (*(list[i])).wordptr != nullptr and 
		   *((*(list[i])).wordptr) == SECRET){
			sum += (*(list[i])).num;
		}
	}
        return sum;
...
int search_array_of_ptrs_to_arrays(Item *list[])
{
	...
	for(int i = 0; i < ARRAY_SIZE; i++){
		...
		while((list[i][j].wordptr) != nullptr){
			if(list[i] != nullptr  and
			   list[i][j].wordptr != nullptr and
			   *(list[i][j].wordptr) == SECRET){
				sum += (*(list[i])).num;
			}
			j++;
		}
